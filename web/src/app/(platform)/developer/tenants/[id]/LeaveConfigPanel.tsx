...
type LeaveTier = "basic" | "standard" | "advanced";

const TIER_META: Record<LeaveTier, { label: string; badge: string; badgeClass: string; desc: string; icon: React.ElementType }> = {
  basic:    { label: "Basic",    badge: "Tier 1", badgeClass: s.tierBadgeBasic,    desc: "2 leave types. Superadmin configures. Single-level approval. No carry-forward.", icon: Leaf },
  standard: { label: "Standard", badge: "Tier 2", badgeClass: s.tierBadgeStandard, desc: "Up to 10 types (EL/CL/SL/LWP+). Carry-forward, partial-day, CL consecutive limit.", icon: BookOpen },
  advanced: { label: "Advanced", badge: "Tier 3", badgeClass: s.tierBadgeAdvanced, desc: "Unlimited types. ML leave + lapse tracking, short leave, CO, week-off customisation, payroll linkage.", icon: Sparkles },
};
  ...
  return (
    <div>
      ...
      <div className={s.tierGrid}>
        {(["basic", "standard", "advanced"] as LeaveTier[]).map(t => {
          const m = TIER_META[t];
          const Icon = m.icon;
          return (
      ...
      <div className={s.section}>
        ...
        <div className={s.fieldGrid1}>
          <Toggle label="Allow Carry-Forward" locked={!isStd}
            hint="Unused leave balance rolls over to next year (configurable % per leave type)"
            field="allow_carryforward" value={!!props.allow_carryforward} onChange={set} />

          <Toggle label="Partial Day Support (Half-Day Leave)" locked={!isStd}
            hint="Employees can apply for half-day leave for applicable leave types"
            field="partial_day_support" value={!!props.partial_day_support} onChange={set} />

          <Toggle label="CL Consecutive Day Limit" locked={!isStd}
          ...
          {!!props.cl_consecutive_limit_enabled && isStd && (
      ...
      <div className={s.section}>
        ...
        <div className={s.fieldGrid1}>
          <Toggle label="Menstruation Leave (ML) Type" locked={!isAdv}
            hint="Allows HR to create an ML leave type with unused-month lapse tracking"
            field="ml_leave_enabled" value={!!props.ml_leave_enabled} onChange={set} />

          {!!props.ml_leave_enabled && isAdv && (
            <div className={s.subField}>
              <div className={s.fieldGrid}>
                ...
                <OtherSelect field="ml_lapse_award_type"
                  ...
                  label="Bonus Leave Type" locked={!isAdv} onChange={set} />
              </div>
            </div>
          )}

          <Toggle label="Short Leave (N-Hour Leave)" locked={!isAdv}
          ...
          {!!props.short_leave_enabled && isAdv && (
            <div className={s.subField}>
          ...
          <Toggle label="Comp-Off (Compensatory Off)" locked={!isAdv}
          ...
          <Toggle label="Week-Off Customisation" locked={!isAdv}
          ...
          <Toggle label="Link LWP to Payroll Deduction" locked={!isAdv}
          ...
          <Toggle label="Deficit Adjustment Pool" locked={!isAdv}
