...
type ProfilesTier = "basic" | "standard" | "advanced";

const TIER_META: Record<ProfilesTier, { label: string; badge: string; badgeClass: string; desc: string; icon: React.ElementType }> = {
  basic:    { label: "Basic",    badge: "Tier 1", badgeClass: s.tierBadgeBasic,    desc: "Only SuperAdmin creates/edits profiles. Manager and Admin are read-only.", icon: UserCheck },
  standard: { label: "Standard", badge: "Tier 2", badgeClass: s.tierBadgeStandard, desc: "SuperAdmin + Admin manage profiles. Manager can view team but not edit.",   icon: ShieldCheck },
  advanced: { label: "Advanced", badge: "Tier 3", badgeClass: s.tierBadgeAdvanced, desc: "Custom job roles. Granular field permissions. Salary change approval. Team-scoped manager access.", icon: Crown },
};
  ...
  return (
    <div>
      ...
      <div className={s.tierGrid}>
        {(["basic", "standard", "advanced"] as ProfilesTier[]).map(t => {
          const m = TIER_META[t];
          const Icon = m.icon;
          return (
            <div key={t} className={`${s.tierCard} ${tier === t ? s.active : ""}`}
              onClick={() => {
                ...
                if (t === "basic") {
                  base.who_can_create_profiles = "superadmin";
                  base.who_can_edit_profiles = "superadmin";
                  base.admin_can_create = false;
                  base.admin_can_edit = false;
                }
                if (t === "standard") {
                  base.admin_can_create = true;
                  base.admin_can_edit = true;
                }
                onChange(base);
              }}>
      ...
      <div className={s.section}>
        ...
        <div className={s.fieldGrid1} style={{ marginTop: 12 }}>
          ...
          <Toggle label="Admin Can Create Profiles" locked={!isStd}
            hint="Basic tier: only SuperAdmin can create. Standard+ allows Admin."
            field="admin_can_create" value={!!props.admin_can_create} onChange={set} />

          <Toggle label="Admin Can Edit Profiles" locked={!isStd}
      ...
      <div className={s.section}>
        ...
        <div className={s.fieldGrid1}>
          ...
          {!!props.custom_job_roles_enabled && isAdv && (
