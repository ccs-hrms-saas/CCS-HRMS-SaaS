...
type PayrollTier = "basic" | "standard" | "advanced";

const TIER_META: Record<PayrollTier, { label: string; badge: string; badgeClass: string; desc: string; icon: React.ElementType }> = {
  basic:    { label: "Basic",    badge: "Tier 1", badgeClass: s.tierBadgeBasic,    desc: "Fixed-day calculation. Leaves + holidays reduce LWP. Simple monthly payout.", icon: Zap },
  standard: { label: "Standard", badge: "Tier 2", badgeClass: s.tierBadgeStandard, desc: "Hours-based calc, deficit tracking, pay-day lock, superadmin override.",         icon: BarChart3 },
  advanced: { label: "Advanced", badge: "Tier 3", badgeClass: s.tierBadgeAdvanced, desc: "Shift-based, differential rules per employee group, ML lapse reward engine.",     icon: BrainCircuit },
};
...
export default function PayrollConfigPanel({ props, onChange, onSave, saving }: Props) {
  ...
  // Merge helper — mutates props state via onChange
  ...
  return (
    <div>
      ...
      <div className={s.tierGrid}>
        {(["basic", "standard", "advanced"] as PayrollTier[]).map(t => {
          const m = TIER_META[t];
          const Icon = m.icon;
          return (
            <div
              ...
              onClick={() => {
                ...
                // Auto-unlock relevant defaults when upgrading
                if (t === "standard") { base.deficit_tracking = base.deficit_tracking ?? false; }
                if (t === "advanced") { base.shift_based_calc = base.shift_based_calc ?? false; base.ml_lapse_tracking = base.ml_lapse_tracking ?? false; }
                onChange(base);
              }}
            >
              <div>
      ...
      <div className={s.section}>
        ...
        <div className={s.fieldGrid1}>
          <Toggle label="Auto-Compute LWP from Attendance Hours" locked={!isStd}
          ...
          <Toggle label="Hard Lock Payroll After Pay Day" locked={!isStd}
            hint="Record becomes immutable on/after the Pay Day — prevents accidental edits"
            field="payroll_lock_enabled" value={!!props.payroll_lock_enabled} onChange={set} />

          <Toggle label="Enable Hour Deficit Tracking" locked={!isStd}
      ...
      <div className={s.section}>
        ...
        <div className={s.fieldGrid1}>
          <Toggle label="Shift-Based Calculation" locked={!isAdv}
            hint="Enable multiple shift patterns with different hour targets per employee"
            field="shift_based_calc" value={!!props.shift_based_calc} onChange={set} />

          <Toggle label="Differential Rules Per Employee/Group" locked={!isAdv}
            hint="Different working hour rules for different departments or employee groups"
            field="differential_rules_enabled" value={!!props.differential_rules_enabled} onChange={set} />

          <Toggle label="Weekly Overtime Cap" locked={!isAdv}
          ...
          {props.overtime_weekly_cap !== null && isAdv && (
            <div className={s.subField}>
              <NumField label="Weekly Overtime Cap (hrs)" field="overtime_weekly_cap"
                value={props.overtime_weekly_cap ?? 5} min={1} max={20} step={0.5}
                locked={!isAdv} onChange={set} />
            </div>
          )}

          <Toggle label="ML Lapse Reward Engine" locked={!isAdv}
            hint="Track unused Menstruation Leave months and auto-credit bonus leave"
            field="ml_lapse_tracking" value={!!props.ml_lapse_tracking} onChange={set} />

          {!!props.ml_lapse_tracking && isAdv && (
            <div className={s.subField}>
              <div className={s.fieldGrid}>
                ...
                <OtherSelect field="ml_lapse_award_type"
                  ...
                  label="Award Leave Type" locked={!isAdv} onChange={set} />
              </div>
            </div>
          )}
        </div>
