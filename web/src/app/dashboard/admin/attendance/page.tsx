...
interface DayRow {
  ...
  checkIn?: string;
  checkOut?: string;
  checkinPhoto?: string;    // photo_url from DB
  checkoutPhoto?: string;   // checkout_photo_url from DB
  status: EmpStatus;
    ...
    <div className="animate-fade-in">
      ...
      <div className={`glass-panel ${styles.tableWrap}`}>
        <table>
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={7} style={{ textAlign: "center", padding: 32 }}><div className={styles.spinner} style={{ margin: "0 auto" }} /></td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={7} style={{ textAlign: "center", color: "var(--text-secondary)", padding: "32px" }}>No records found.</td></tr>
            ) : filtered.map((row) => (
