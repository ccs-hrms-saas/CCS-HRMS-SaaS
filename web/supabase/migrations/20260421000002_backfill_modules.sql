...
-- Safe to re-run (ON CONFLICT / WHERE NOT EXISTS = idempotent)
--
-- IMPORTANT: Uses SECURITY DEFINER to bypass RLS so the backfill reaches
-- every company in the database, not just those visible to the current
-- session. This is the required pattern for SaaS-wide module provisioning.
-- ══════════════════════════════════════════════════════════════════════════

CREATE OR REPLACE FUNCTION public._backfill_capability_expansion()
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  WHERE NOT EXISTS (
    ...
    WHERE cm.company_id = c.id AND cm.module_key = 'reimbursements'
  );
  ...
  WHERE NOT EXISTS (
    SELECT 1 FROM public.company_modules cm
    WHERE cm.company_id = c.id AND cm.module_key = 'profiles'
  );

  -- ── 3. Backfill payroll with tier properties (merge, don't overwrite) ────
  ...
  WHERE module_key = 'payroll'
    AND NOT (properties ? 'tier');

  -- ── 4. Backfill leave_settings with tier properties ──────────────────────
  UPDATE public.company_modules
  SET properties = '{
    "tier": "basic",
    "max_leave_types": 2,
...
-- Execute then self-clean
SELECT public._backfill_capability_expansion();
DROP FUNCTION public._backfill_capability_expansion();
